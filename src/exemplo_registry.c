...
int _tmain(int argc, TCHAR* argv[]) {
    ...
    _tprintf(TEXT("Qual � o nome da chave: "));
    ...
    /*Entra no ciclo de cria��o de pares-valores*/
    do {
        ...
        if (RegSetValueEx(
                ...
                REG_SZ,
                (LPCBYTE)&par_valor,
                sizeof(TCHAR) * (_tcslen(par_valor) + 1) //l� o buffer todo
        ) != ERROR_SUCCESS)
            _tprintf(TEXT("O atributo nao foi alterado nem criado! ERRO! %s"), par_nome);
    } while (1);


    /*Entra no ciclo de verifica��o de valores dos pares-valores*/
    do {
        TCHAR nome[TAM];
